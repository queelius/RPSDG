    ...
    def match(self, expr: Expression, pattern: Expression, bindings: dict) -> bool:
        if isinstance(pattern, Variable):
            ...
            if pattern.op == expr.op:
                if isinstance(pattern.left, Variable) and isinstance(pattern.right, Variable) and pattern.left.name == pattern.right.name:
                    return self.match(expr.left, pattern.left, bindings) and self.match(expr.right, pattern.left, bindings)
                return (self.match(expr.left, pattern.left, bindings) and self.match(expr.right, pattern.right, bindings))
...
test_expressions = [
    BinaryOp("*", BinaryOp("+", Number(2), Number(2)), Number(4)),
    BinaryOp("*", BinaryOp("+", Variable("y"), Variable("y")), Number(4)),
