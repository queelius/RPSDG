    ...
    def match(self, expr: Expression, pattern: Expression, bindings: dict) -> bool:
        ...
        if isinstance(pattern, Variable):
            ...
            if pattern.op == expr.op:
                # Ensure that the pattern matches both sides correctly
                if pattern.left == pattern.right:
                    if self.match(expr.left, pattern.left, bindings) and expr.left == expr.right:
                        return True
                else:
            ...
            return pattern.op == expr.op and self.match(pattern.expr, expr.expr, bindings)
        return False
...
class GeneralRule(Rule):
    def match(self, expr: Expression, pattern: Expression, bindings: dict) -> bool:
        if debug:
            print(f"Matching {expr} with {pattern}")
        if isinstance(pattern, Variable) and pattern.name == "Repeated":
            if isinstance(expr, BinaryOp) and expr.op == pattern.op:
                count = 1
                current = expr
                while isinstance(current, BinaryOp) and current.op == pattern.op:
                    if current.right == pattern.left:
                        count += 1
                        current = current.left
                    else:
                        break
                if count > 1:
                    bindings["count"] = Number(count)
                    bindings[pattern.left.name] = current
                    return True
        return super().match(expr, pattern, bindings)

    def instantiate(self, template: Expression, bindings: dict) -> Expression:
        if isinstance(template, BinaryOp) and template.op == "Repeated":
            count = bindings["count"].value
            term = bindings[template.left.name]
            return BinaryOp("*", Number(count), term)
        return super().instantiate(template, bindings)
...
simple_rules = [
    ...
    GeneralRule("Rewrite n*a as Repeated",
         BinaryOp("+", Variable("a"), Variable("a")),
         BinaryOp("*", Variable("count"), Variable("a"))),
    EvalRule("Eval", Variable("X"), Variable("X"), evaluate=True)
]
...
test_expressions = [
    ...
    BinaryOp("+", Variable("a"), BinaryOp("+", Variable("a"), BinaryOp("+", Variable("a"), Variable("a"))))
]
