        ...
        if isinstance(pattern, Variable):
            ...
            if pattern.op == expr.op:
                ...
                if isinstance(pattern.left, Variable) and isinstance(pattern.right, Variable) and pattern.left.name == pattern.right.name:
                    match_left = self.match(expr.left, pattern.left, bindings)
                    match_right = self.match(expr.right, pattern.left, bindings)
                    print(f"Special case: matching {expr.left} and {expr.right} to {pattern.left.name}")
